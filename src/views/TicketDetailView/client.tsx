  ...
  return (
    <div className={s.page}>
      ...
      <div className={s.topBar}>
        <Link href="/admin/support/inbox" className={s.backLink} aria-label={t('detail.backToInbox')}>&larr;</Link>
                ...
                <div className={s.uploadPreview}>
                  {pendingFiles.map((f, i) => (
                    <div key={i} className={s.uploadPreviewItem}>
                      ...
                      <button className={s.uploadRemoveBtn} aria-label={t('detail.removeUpload', { name: f.name })} onClick={() => setPendingFiles((prev) => prev.filter((_, j) => j !== i))}>&times;</button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className={s.tagsWrap}>
              {tags.map((tag) => (
                <span key={tag} className={s.tagChip}>
                  {tag}
                  <button className={s.tagRemove} aria-label={t('detail.removeTag', { tag })} onClick={() => handleRemoveTag(tag)}>&times;</button>
                </span>
              ))}
              {addingTag ? (
                <input
